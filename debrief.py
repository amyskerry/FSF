...
cursor.execute('update FSF_rating set age="'+age+'" where rownum="'+formindex+'"')
cursor.execute('update FSF_rating set gender="'+gender+'" where rownum="'+formindex+'"')
cursor.execute('update FSF_rating set submitdate="'+datevar+'" where rownum="'+formindex+'"')


for x in ['country','city','thoughts']:
    try:
        it = form[x].value
        cursor.execute('update FSF_rating set '+x+'="'+it+'" where rownum="'+formindex+'"')
    except: pass
